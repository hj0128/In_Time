...
describe('Party_UserController (e2e)', () => {
  ...
  describe('setting', () => {
    ...
    it('K -> J (친구 신청)', async () => {
      const inputFriendCreateDto: FriendCreateDto = { toUserName: 'J' };
  ...
  describe('Party_User e2e test', () => {
    it('/partyUser/partyUserFindAllWithPartyID (Get)', async () => {
      const inputPartyID: string = partyID;

      const response = await request(app.getHttpServer())
        .get('/partyUser/partyUserFindAllWithPartyID')
        .set('Authorization', `Bearer ${accessToken}`)
        .set('Cookie', `refreshToken=${refreshToken}`)
      ...
      expect(response.body[0].party.id).toBe(inputPartyID);
    });
  });
});
